...
tests: desktop 1035, harness 113 (+2 skipped), contracts 159 - green on the STEP-23-04 branch
...
blockers: none
in_flight: 'Phases 21-22 executed and merged; refined plans for phases 23-29 merged; Phase 23 execution underway - STEP-23-01/02/03 merged to main (PRs #21/#22/#23), STEP-23-04 completed on phase/23-step-04-composer, STEP-23-05 next'
...
- Also active: 46 more additional sessions, 61 more additional steps, 1 open critical bug.
...
- **Resume point (single source of truth)**: continue Phase 23 execution at [[02_Phases/Phase_23_chat_ui_v1_over_ephemeral_acp_sessions/Steps/Step_05_add-mock-agent-driven-chat-e2e-coverage|STEP-23-05 Add mock-agent-driven chat E2E coverage]]. STEP-23-01 (chat panel, `chat:session:*` IPC, transcript reducer, GFM rendering — PR #21), STEP-23-02 (tool-call cards, diffs, terminal embeds, plan panel, client services — PR #22) and STEP-23-03 (real permission round-trips: pure engine, `chat:permission:*` IPC, prompt UI, trust badge — PR #23) are all completed and merged to `main`. STEP-23-04 (composer with slash commands, session modes, cancel and crash/error surfaces) is completed on `phase/23-step-04-composer` and awaiting its PR. Refine is finished for phases 23-29.
- Carry-forward into STEP-23-05: the fixed `MOCK_DEMO_SCENARIO` in `packages/desktop/src/main/chat/session-controller.ts` is to be replaced by injectable scenarios. Keep its `initialize.modes` and `advertise_commands` directive alive in whatever replaces it, or the manual `pnpm dev` path loses the mode selector and the slash menu.
- Owed against STEP-23-04 (verification, not implementation): a manual `pnpm dev` walkthrough against the mock, and a manual real-Pi run (mode switch, cancel a long turn, externally kill `pi-acp` to confirm the crash banner and recovery).
