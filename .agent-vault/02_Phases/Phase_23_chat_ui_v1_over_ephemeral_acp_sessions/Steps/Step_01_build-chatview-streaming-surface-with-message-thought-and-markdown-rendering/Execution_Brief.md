...
- `transcriptReducer.ts` — a pure reducer folding raw `session/update` notifications into a transcript model: `agent_message_chunk` / `agent_thought_chunk` / `user_message_chunk` append to the current message/thought block; `tool_call` / `tool_call_update` / `plan` / `available_commands_update` / `current_mode_update` are stored for steps 02–04 (stub entries fine this step). **Tolerant reader rule (ARCH-0009):** unknown update kinds are ignored, never a crash — the spike observed `session_info_update`, which the mock does not even script.
