...
- **"Always" memory key (Decision needed — recorded, non-blocking):** ACP defines no rule key for remembering `allow_always`. A bare `(sessionId, toolCall.kind)` key is NOT acceptable — it would let one `allow_always` for a single `execute` or `edit` authorize every later command or path in the session. Default for this phase: key on `(sessionId, toolCall.kind, normalizedScope)` where `normalizedScope` is derived from the normalized request — for `edit`/`read`/`delete`/`move`: the canonical affected path; for `execute`: the command's program token (first argv word); otherwise a title-prefix fallback. If no scope is derivable from the request, the engine MUST NOT silently broaden: treat the `allow_always` as scoped to that underivable-title key, or (UI option) let the user explicitly confirm a kind-wide "always allow all `<kind>`" — an explicit broad-scope confirmation is the only path to kind-wide reuse. Executor picks the default unless the human overrides; record the choice in Implementation Notes. Memory is per-session and dies with the session (ephemeral phase; Phase 24 revisits persistence + project policy).
...
- **Pending-request deadline:** cancel and window-destruction cleanup are not enough — a lost IPC response or wedged renderer must not leave the agent awaiting a resolver forever. Every pending-request entry gets a deadline timer (default 10 minutes, injectable clock for tests); on expiry the port resolves that request `cancelled`, removes the `requestId` entry, dismisses/expires the prompt UI, and emits the decision audit event with an `expired` reason. A response arriving after resolution (late, duplicate, or post-expiry) is ignored with a warning — the pending map is the single source of truth for liveness.
