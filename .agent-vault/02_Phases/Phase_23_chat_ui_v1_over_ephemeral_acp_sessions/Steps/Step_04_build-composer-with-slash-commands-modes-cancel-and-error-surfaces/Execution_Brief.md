...
- **Exact IPC payload contracts (define these, do NOT lean on `Schema.Unknown`):** add both new chat channels to `packages/contracts/src/ipc/contracts.ts` as typed effect/Schema structs following the `SDevSession*`/`SChatSession*` pattern, and require preload/controller round-trip tests that decode a valid payload and reject a malformed one:
  - `chat:session:set-mode` — request `{ sessionId: string, modeId: string }` (renderer→main; `parseSync` at the boundary), response `{ ok: true, currentModeId: string }` or a typed error. The controller maps `modeId` → `connection.setMode`; an unknown `modeId` (not in the session's advertised modes) is rejected before the ACP call.
  - `chat:session:status` — main→renderer push `{ sessionId: string, status: 'spawning' | 'ready' | 'crashed' | 'gave-up' | 'exited', stderrTail?: string, exitCode?: number | null, message?: string }`. `status` mirrors the `SupervisorEvent` union (minus the reducer-internal `reaped`); `stderrTail` is populated only for `crashed`/`gave-up` (capped length — carry the `SupervisorGaveUp` tail). Renderer keys these by `sessionId` (drop events for a stale handle, mirroring STEP-23-01's `sessionIdRef` guard).
  - Preload guards: expose `chatSessionSetMode(payload)` and `onChatSessionStatus(cb)` on `window.srgnt` with the same channel-name allowlist + `parseSync`-on-receive pattern the existing push channels use — no raw `ipcRenderer` passthrough, no `Schema.Unknown` escape hatch.
...
- Crash/restart surface: subscribe the chat controller to `supervisor.onEvent` and push `crashed`/`gave-up` (with stderr tail from `SupervisorGaveUp`) to the renderer over the `chat:session:status` push channel defined above (typed `{ sessionId, status, stderrTail?, exitCode?, message? }` — never `Schema.Unknown`). Renderer shows a recoverable error state with a "New session" affordance. **Ephemeral-phase reality (assumption):** restart = dispose + fresh `session/new`; transcript of the dead session stays visible read-only until the user starts the new one. `session/load`-based restore is Phase 24 (even though Pi supports `session/load`, there is no persistence to reload from this phase).
