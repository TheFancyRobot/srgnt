...
- **Tolerant read (ARCH-0009 failure mode):** read line-by-line; a line that fails `JSON.parse` or `readSessionEvent` *at the tail* is dropped and reported (`{events, truncatedTail: true, lastValidByteOffset}`) — never a throw. Unknown `kind` values decode fine by design. A structurally-bad line in the *middle* of the file (any content after the last valid record's newline that isn't itself a recoverable trailing partial) is beyond the crash model — surface a typed error rather than silently skipping (Decision needed only if this ever occurs in practice; recorded).
- **Repair-before-append (corrupt-tail recovery):** dropping a bad tail *only at read time* is not enough — if the next event is appended after an unterminated/malformed trailing line, that corrupt line becomes an *interior* line and every future read hits the middle-corruption error path. So on open, recovery MUST record the byte offset immediately after the last valid record's newline and, before permitting any append, **truncate the file to that offset** (`ftruncate`/reopen at the boundary) so writing resumes from a clean end. Only then is the append handle opened. Seq continues densely from the last valid event — no gap, no reuse.
...
5. Property tests (fast-check): arbitrary event sequences round-trip append→read identically (unicode, nested payloads, unknown kinds); arbitrary truncation of the final line still yields all prior events + `truncatedTail: true`; interleaved concurrent appends preserve per-session seq density. **Reopen → repair → append → read cycle:** corrupt/partial the tail of a written log, reopen the store, append a new event, then read — assert the corrupt line is gone, the new event persisted, sequence numbers are continuous (no gap or reuse across the repair boundary), and a subsequent reopen reads cleanly (the repaired file never trips the middle-corruption error).
