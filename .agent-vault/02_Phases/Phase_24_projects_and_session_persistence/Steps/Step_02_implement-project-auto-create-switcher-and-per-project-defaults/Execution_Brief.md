...
- Auto-create must be idempotent and race-safe: two concurrent `ensureProjectForDir` calls for the same dir yield one project (the deterministic id makes the second `mkdir`/write a benign no-op — use `wx`/EEXIST tolerance like `bootstrapWorkspace`).
...
2. Build `packages/runtime/src/projects/` store with unit tests: auto-create idempotency, stable-id-by-rootDir, rename keeps id, merge semantics (session dirs moved, additionalDirectories unioned, source removed), path-unsafe input rejection.
