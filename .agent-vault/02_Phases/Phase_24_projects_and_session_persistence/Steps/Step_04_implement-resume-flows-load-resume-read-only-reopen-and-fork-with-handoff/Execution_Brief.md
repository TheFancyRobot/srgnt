...
- `packages/desktop/src/main/chat/` — the reconnect flow, triggered on the first prompt of a reopened session (lazy: reopening spawns nothing):
  ...
  3. Then send the pending prompt. Failure of load/resume itself (e.g. the harness lost the session server-side) degrades to read-only + fork with a visible notice — never a fake continue.
- **Replay reconciliation (recorded assumption — keep v1 simple):** the local log REMAINS canonical. Replayed updates during `load` are consumed but NOT re-appended (no duplicate events); the renderer keeps its locally-rendered transcript. Reconciliation v1 = count + last-update comparison between replayed `session/update` frames and persisted `acp/session_update` events; on mismatch, append a `client/load_reconciliation` event recording both counts and show a subtle "history may differ on the agent side" notice. Decision needed (non-blocking): whether mismatch should ever *replace* the local render — default is NO (source-of-truth invariant).
...
- `packages/contracts/src/ipc/contracts.ts` — `chat:session:fork` (sourceSessionId, optional include-handoff flag → returns the new session ref + prefilled text); read-only state surfaced through the existing session-status push.
...
3. Implement the reconnect flow with the capability branch + reconciliation; unit tests with in-process mocks in all three capability configurations (`resumeSession: true`, `loadSession: true` with matching and mismatching replays, neither).
4. Implement fork-with-handoff (service + IPC + deterministic handoff template with unit tests).
