...
- Crash-mid-turn E2E: mock scenario streams then `crash` → app restarted (fresh Electron launch, same workspace temp dir) → session listed as `interrupted`, transcript (re-rendered from `events.jsonl` on open, NOT read from a stale checkpoint) shows everything up to the last received update, only the in-flight chunk missing; a truncated/partial log tail is tolerated. The bound holds regardless of how long before the crash the last 30 s transcript checkpoint ran, because recovery derives the transcript from the durable per-event log.
- Idle reaping: with a short injected timeout, an idle session's process is reaped (`reaped {reason:'idle'}` audit event in the log; `ps` clean) and the session stays `idle` in the list; the next prompt respawns transparently through the 04 reconnect flow. A reap can NEVER fire during an active turn — fake-clock unit test drives a SILENT in-flight prompt (a turn that streams no updates for longer than `idleTimeoutMs`) and asserts it is not reaped, proving correctness comes from the disarm-on-turn-start / re-arm-on-turn-end transition, not from `markActivity` heartbeats.
- Quit cleanup: quitting with (a) idle sessions, (b) an active turn, (c) a crashed-and-backing-off session leaves ZERO agent child processes (process-tree assertion via `ps` in E2E teardown); in-flight turns received a best-effort `session/cancel` (mock `expect_cancel` assertion) and the ENTIRE cleanup sequence (cancel + final checkpoint + `disposeAll`) completed within one overall budget — a unit test with a hanging cancel asserts total wall-clock stays within the single deadline and `disposeAll` still runs as the backstop.
...
- `session/cancel` hanging at quit (mock never responds) → the single overall 2 s budget expires, the final checkpoint and kill-tree (`disposeAll`) still run within/after the deadline as best-effort + guaranteed backstop, app exits.
