...
- Two rapid connects for the same harness → the **newest negotiation** wins without a torn file. First assert the allocation itself: two `reserve('pi')` calls issued concurrently (before either write lands) return **distinct, strictly increasing** generations — a test that lets both read the stored value and compute `stored + 1` must fail, since equal generations make completion order the tiebreaker again. Then the concurrency unit test must deliberately complete a stale writer *after* a newer one (out-of-order async completion) and assert the stale write is rejected by the store's `generation` check — asserting only "file not torn" does not cover this. Cover the two cases `capturedAt` cannot decide: (a) both writers stamped with an **identical** `capturedAt` (frozen or low-resolution clock) → the lower `generation` still loses; (b) the later writer stamped with an **earlier** `capturedAt` than the stored entry (simulated clock rollback) → it still wins if its `generation` is higher, and the stored `capturedAt` is updated with it. Also assert generations survive a restart: reopening the store and connecting again allocates `storedGeneration + 1`, never restarts at 0.
