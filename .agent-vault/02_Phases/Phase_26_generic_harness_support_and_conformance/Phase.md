...
- Docs: "Add your own harness" guide with worked examples for Gemini CLI, claude-acp, and codex-acp.
...
- Requires PHASE-25's lessons-learned note — [[06_Shared_Knowledge/cross-harness-lessons-learned|Cross-Harness Lessons Learned (STEP-25-04)]] — as the requirements input, and its capability matrix/settings surfaces to extend. **Delivered 2026-08-15:** REQ-26-01…18, each carrying an evidence pointer and a deliverable mapping (editor / runner / catalog / docs), plus PROP-A…D. **The four proposals are held back for different reasons, and flattening them into "unowned" would drop real work:**
  - **PROP-A — a generic `session/set_config_option` surface. MEASURED 2026-08-15, so this is now purely a scope call.** The method works against opencode 1.18.18 with `{sessionId, configId, value}` and returns the updated `configOptions`. The ACP spec is explicit that **config options supersede session modes** and that dedicated mode methods will be removed in a future protocol version, so srgnt's `readModes`/`session/set_mode` path is on the deprecated side of a transition, not merely incomplete. **SCOPE DECIDED 2026-08-15: PHASE-26 reports, a later phase drives.** Raised as [[04_Decisions/DEC-0021_adopt-acp-session-config-options-and-retire-the-modes-path|DEC-0021]], **accepted 2026-08-16** and owned by [[02_Phases/Phase_30_session_config_options/Phase|PHASE-30]]. PHASE-26 keeps only REQ-26-18's reporting, with the caveat that srgnt does not advertise `session.configOptions.boolean` — so a missing boolean option measures srgnt, not the agent. Advertising it early would be a capability lie by this codebase's own rule (`connection.ts` advertises from implementation presence). Rationale for not absorbing it: PHASE-26 is already editor + runner + registry + docs, and this is protocol work with its own UI surface that would blur two steps not written for it.
  - **PROP-B — `session/close` and `session/fork`. MEASURED 2026-08-15: both work against opencode, but only one is stable protocol.** `session/close` is stable v1 (gated by `sessionCapabilities.close`) and safely promotable — srgnt currently tears sessions down by killing the process instead. **`session/fork` is an open RFD**, absent from the v1 docs and from the maintainers' capability list; opencode implemented it early. A working probe against one agent is not evidence the surface is settled, so report it rather than depend on it. `session/delete` also exists in the protocol and srgnt models it nowhere.
...
- [ ] A user can add a custom harness (command/args/env + overrides) through the UI and run a session with it — proven with at least one non-built-in real agent (e.g., Gemini CLI or claude-acp).
...
- [[04_Decisions/DEC-0021_adopt-acp-session-config-options-and-retire-the-modes-path|DEC-0021 Adopt ACP session config options and retire the session-modes path]] (ACCEPTED 2026-08-16) — raised out of this phase's refinement. PHASE-26 does **not** implement it: the runner reports config options and flags them unreachable, and DEC-0021 owns the driving surface for a later phase. Read it before building any mode/model selector.
...
- Candidate third-party validation targets, **verified present in the live ACP registry 2026-08-15**: `gemini` "Gemini CLI" (`npx @google/gemini-cli@0.55.1 --acp`, production-grade reference agent), `claude-acp` "Claude Agent", `codex-acp` "Codex" (`npx @agentclientprotocol/codex-acp@1.3.0`). **These notes previously named `claude-code-acp`, which is not a registry entry** — corrected throughout. Version pins are as-of that date; re-verify at execution.
- **Refinement pass 2026-08-15 (post-gate).** The information gate below is now OPEN: `06_Shared_Knowledge/cross-harness-lessons-learned.md` exists with REQ-26-01…18. Each step's brief has been reconciled against the delivered list, and two external facts the earlier pass could only guess at are now verified:
  - **The ACP registry feed exists** — `https://cdn.agentclientprotocol.com/registry/v1/latest/registry.json`, schema in `agentclientprotocol/registry` → `FORMAT.md`. STEP-26-03 is a real integration, not a snapshot-only step. Its `distribution` object has three kinds (`npx`, `uvx`, `binary`), and **`binary` entries require download+extract, which collides with this phase's "srgnt never installs" non-goal** — see STEP-26-03's brief for the recorded default and the open question.
  - **`claude-code-acp` is not in the registry**; the entry is `claude-acp`. Corrected everywhere.
  - New editor work the earlier pass did not anticipate, from delivered REQs: **per-entry tolerance in `harnesses.json` (REQ-26-06)** — verified that `loadWorkspaceHarnesses` currently fails the whole file on one bad entry (`registry.ts:34`); **stale-on-save (REQ-26-05)**; and **REQ-26-07 is a forced decision** — keeping wholesale-shadow now *requires* UI copy stating the consequence.
  - Defaults that now hold because no REQ demanded otherwise: no `launch.cwd` templating; no per-harness permission-policy defaults (that is PROP-C, outside the REQ list).
- Superseded refinement pass 2026-07-18 (pre-gate; the "expect REQ-26-xx to refine this" markers in each brief have been resolved rather than removed, so the reasoning stays auditable). This phase sat DOWNSTREAM of a designed information gate. Briefs are junior-deep where shipped code determines the plan (contracts/registry/detect/mock-agent/spike prior art) and carry explicit "expect REQ-26-xx to refine this" markers where the lessons note gates specifics — every step's checklist item 1 is "read the lessons note and reconcile":
