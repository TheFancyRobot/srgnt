...
- Report contract: `SConformanceReport` in `packages/contracts/src/` (assumption — it crosses IPC to Settings, so it is an app contract like `SHarnessesFile`): per-check `{ id, title, status: pass|fail|not-supported|skipped, evidence?, detail? }`, plus negotiated capability row, agent info, suggested quirks, timings, `ranAt` + definition snapshot.
- Check → ACP method map (v1 catalog; confirm against REQ-26-xx):
  1. **detect** — `detectHarness(definition)` → `not-installed` short-circuits everything else to `skipped` (not failed).
  ...
  3. **auth advertisement** — `authMethods` from the negotiated row; if `session/new` later fails with the verified auth-required shape, report `not-supported`-style "auth required" and mark downstream checks `skipped` — an unauthenticated harness is a *finding*, not a runner failure.
  ...
  8. **permission round-trip (behavioral, deep)** — instrumented `PermissionPort`; a permission-inviting prompt; no `session/request_permission` observed → suggest `permission-routing-gaps` (spike probe 1).
