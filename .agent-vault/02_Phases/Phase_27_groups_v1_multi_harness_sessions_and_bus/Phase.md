...
- Refinement pass 2026-07-18 (post-DEC-0018 reconciliation; details live in each step's Execution Brief):
  ...
  - Decision needed (non-blocking, defaults recorded in the briefs): Attach-Group handoff auto-send on create (default: yes — user composed it in the same dialog; distinct from Phase-24's never-auto-sent machine prefill); tier-2 member *reply* ingestion (mailbox-out watcher) deferred unless Phase 28 pipelines demand mechanical replies; Windows named-pipe path untested on this darwin machine (follow-up).
