...
- Built-in templates: `implement → review → QA → iterate` and `research → implement → test` (generalizing this repo's own pi-team loops); template import/export as JSON (the only supported interchange format in v1 — see the format decision below).
...
- Refinement pass 2026-07-20 (companions filled to junior depth; details live in each step's Execution Brief + Validation Plan):
  ...
  - **Built-ins origin cited** (STEP-28-04): `implement → review → QA → iterate` maps from `docs/pi-teams.md` (`review-team`/`bugfix`/`qa`, `QA REVIEW REQUESTED` handoff) and `research → implement → test` from `.pi/teams.yaml` `srgnt-team` (three stages — the `srgnt-team` reviewer role is intentionally omitted so this built-in stays the minimal fully automatic loop; Built-in 1 already ships the reviewed, gated flow) — concrete role→member + token→condition table in the STEP-28-04 brief (tokens: `RESEARCH BRIEF`, `REVIEW COMPLETE`, `ISSUE REPORT`, `QA REVIEW REQUESTED`). Built-ins ship harness-agnostic (member `harnessId` is a picker-filled placeholder; start blocks until every member is mapped).
  - **Format decision**: **JSON is the only interchange format in v1** — canonical on disk, on export, and on import. No YAML path and no `yaml` dependency (adding one for import-only is out of v1 scope); a YAML file is rejected with "unsupported format, convert to JSON". Import validates before writing and **always refuses** an `id` collision (no overwrite, no auto-suffix, no prompt) and never half-parses — no destructive or partial writes; `import(export(t)) === t`.
