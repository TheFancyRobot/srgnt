...
- Exact outcome: a main-process pipeline runner executes Pipeline definitions as a state machine over ACP prompt turns — fill the stage prompt template (prior-stage output + bus context) → `session/prompt` the stage's member → await the completion condition (stop reason / explicit token / user gate) → evaluate transitions incl. loop-backs with `maxIterations` → advance; run state persists so app restarts recover honestly (interrupted turns marked, resumable where the harness supports load).
