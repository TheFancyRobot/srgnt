...
- **Every stage entry deep-links**: in a completed multi-stage run (including a loop-back revisit and a short prompt well under the size cap), *every* `stage_entered` for a member-running stage carries an `inputRef` that resolves to a real record in that member's `events.jsonl`. Assert the ref is present on the untruncated entries too — the truncated case must not be the only one covered — and that gate-stage entries legitimately carry none.

## Edge Cases

- Restart mid-stage on a **non-gate** stage: `stage_entered` with no completion and no `gate_awaiting` → member's last turn marked `interrupted`; run resumable only if the member's harness reports load/resume, else surfaced interrupted-not-resumable (no false "running").
- **Replay precedence (the pending-gate/interrupted distinction):** three fixture logs, asserted separately — (a) `stage_entered` + `gate_awaiting`, no `stage_completed` → run restores as `gated`, nothing marked interrupted; (b) `stage_entered` + `gate_awaiting` + `gate_resolved` → run folds past the gate and continues; (c) `stage_entered` on a member-running stage with neither → `interrupted`. Also cover the crash-between-appends case: a gate stage's `stage_entered` with no `gate_awaiting` restores as `gated` with `gate_awaiting` re-emitted, never as interrupted.
