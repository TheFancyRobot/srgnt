...
- Renderer — register the board as a group-session tab (beside member tabs + `BusTimeline`). **The sole mount condition is the presence of at least one `system/pipeline_started` event in that group's `bus.jsonl`** — nothing else. Do not consult the template file, `SSession.pipeline`, or any other mutable field: those can change after the fact, which would make the tab appear or disappear for a run that already happened (a template edit must never rewrite history, per the STEP-28-02 snapshot invariant). A group that has never started a run shows no board even if its template declares a pipeline; once a run has started, the board stays available forever, including after the template is edited or deleted. This keeps the mount decision the same kind of thing as everything else on this surface: a fold over persisted events.
...
3. Register the board as a conditional group-session tab keyed **only** off `system/pipeline_started` in the log; component tests over the scripted run states; verify the tab hides for groups with no started run (even when their template declares a pipeline) and stays visible for a started run whose template was later edited or deleted.
